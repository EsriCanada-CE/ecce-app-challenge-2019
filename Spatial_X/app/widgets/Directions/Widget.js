...
///////////////////////////////////////////////////////////////////////////
// Made by: Team Spatial X
// From: University of Waterloo
// Last Modified: 22 February, 2019
//
// Description: Incorperated code enables the user to be able to quickly observe
// the quanitity of to be taken steps for the generated routed path 
// 
// Services: Utilizing ESRI's routing widget to determine the amount of steps a
// user of our application would take so to reach their destination
//
...
var stepCnt = {
  perMeter: 1.31234,     // Average quantity of steps a person takes in 1 meter
  distance: 0            // Total distance in kilometers, set to zero initially
};
// Calculates the amount of steps which will be taken by the user for a given route
// initialized with a value of zero, since a route is generated upon the users request
var calc = {
  steps: 0, 
  dtrSteps: function (stpAmt, aqr) {
    return (parseFloat(Math.round(stpAmt)).toFixed(aqr)).toString() + " ";
  }
};
var router = {
  0: {
...
function routeEST(routeResults) {
  router = routeResults;
  stepCnt.distance = router[0].directions.summary.totalLength;

  calc.steps = (stepCnt.distance * 1000) * stepCnt.perMeter;

  document.getElementById("totalSteps").innerHTML = calc.dtrSteps(calc.steps, 0) + " Steps to Destination";
};
